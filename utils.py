...
def get_embedding_dict(thresholds, inputs_dict, model, tokenizer):
    toplayerdict = {}
    min_samples = float('inf')  # Initialize to infinity for comparison
    ...
    for key in inputs_dict:
        ...
        for input in inputs_dict[key]:
            ...
            next_token_candidates_tensor = outputs[0, -1, :]
        ...
        # Update minimum sample size after processing each key
        for threshold in thresholds.values():
            if len(toplayer[threshold]) < min_samples:
                min_samples = len(toplayer[threshold])
    ...
    return toplayerdict, min_samples
